...
const RecommendedCommunities: React.FC<RecommendedCommunitiesProps> = ({ onJoinCommunity }) => {
  ...
  return (
    <Card className="shadow-card w-full">
      <CardHeader className="px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:py-6">
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="flex items-center text-base sm:text-lg">
            ...
            <span className="flex-1">Recommended for You</span>
          </CardTitle>
          <Button 
      ...
      <CardContent className="px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:pt-0 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-4 sm:py-6">
            <TrendingUp className="h-10 w-10 sm:h-12 sm:w-12 mx-auto text-muted-foreground mb-3 sm:mb-4" />
            <p className="text-muted-foreground mb-2 text-sm sm:text-base">No recommendations yet</p>
            ...
            <Button 
              ...
              variant="outline" 
              size="default"
              className="w-full max-w-full sm:w-auto text-xs sm:text-sm whitespace-normal h-auto min-h-[52px] py-3 px-4 leading-tight"
            >
              <div className="flex items-center justify-center gap-1.5">
                <RefreshCw className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                <span className="text-center">Generate Recommendations</span>
              </div>
            </Button>
          </div>
        ) : (
          <div className="space-y-2 sm:space-y-3">
            {recommendations.slice(0, 5).map((rec) => (
              <div key={rec.community_id} className="border rounded-lg p-2 sm:p-3 hover:shadow-sm transition-shadow">
                <div className="flex flex-col sm:flex-row sm:items-center mb-2 sm:mb-3 gap-2">
                  <h4 className="font-semibold text-xs sm:text-sm flex-1 truncate pr-1">{rec.community.name}</h4>
